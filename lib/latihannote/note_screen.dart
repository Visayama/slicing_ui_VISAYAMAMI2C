...
import 'package:slicing_ui_visayamami2c/latihannote/model_note/model_notes.dart';
...
class _NotesScreenState extends State<NotesScreen> {
  ...
  Future<ModelNotes> fetchNotes() async {
    final response = await http.get(
      Uri.parse('http://192.168.100.100/latNotes/getNotes.php'),
    );
  ...
  Future<void> _deleteNote(String id) async {
    try {
      final response = await http.post(
          Uri.parse('http://192.168.100.100/latNotes/deleteNotes.php'),
          body: {'id':id},
      );
      if (response.statusCode == 200) {
        setState(() {
  ...
  Future<void> _editNote(Datum note) async {
    ...
    if (editConfirmed != null && editConfirmed) {
      try {
        final response = await http.post(
          Uri.parse('http://192.168.100.100/latNotes/updateNotes.php'),
  ...
  Future<void> _addNote() async {
    ...
    if (addConfirmed != null && addConfirmed) {
      try {
        final response = await http.post(
          Uri.parse('http://192.168.100.100/latNotes/addNotes.php'),
          headers: {'Content-Type': 'application/x-www-form-urlencoded'},
          body: {
            'judul': _titleController.text,
            'isi': _contentController.text,
